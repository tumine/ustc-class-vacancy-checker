...
5. **自动选课**：在设置中开启「自动选课」功能，检测到空位后将自动点击选课按钮
6. **后台捡漏**：在检索页面勾选心仪的课程"加入跟踪列表"。在顶部的 **"跟踪列表"** 页面内确保开关处于开启状态，即便退出应用，后台 Worker 仍会通过后台 WebView 自动登录教务系统并注入脚本定期查询选课页面余量，一旦有空位将发出系统通知进行提示
